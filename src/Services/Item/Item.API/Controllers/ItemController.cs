...
namespace Item.API.Controllers
{
    ...
    public class ItemController : ControllerBase
    {
        ...
        {
            IQueryable<Model.Item> returnItems = _context.Items.OrderBy(on => on.Id);
            ...
            List<Model.Item> list = await returnItems.ToListAsync();

            return new ItemPayload(list, count);
        }

        [HttpGet("{id}")]
        public async Task<ActionResult<Model.Item>> GetItem(int id)
        {
